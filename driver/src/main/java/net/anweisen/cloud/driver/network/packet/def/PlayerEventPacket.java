...
public class PlayerEventPacket extends Packet {
	...
	public static PlayerEventPacket forProxyLoginRequest(@Nonnull UUID playerUniqueId, @Nonnull String playerName, @Nonnull PlayerConnection playerConnection) {
		return new PlayerEventPacket(PlayerEventType.PROXY_LOGIN_REQUEST, buffer -> buffer.writeUUID(playerUniqueId).writeString(playerName).writeObject((SerializableObject) playerConnection));
	}

	public static PlayerEventPacket forProxyLoginRequest(@Nonnull CloudPlayer player) {
		return new PlayerEventPacket(PlayerEventType.PROXY_LOGIN_REQUEST, buffer -> buffer.writeObject((SerializableObject) player));
	}

	public static PlayerEventPacket forProxyLoginSuccess(@Nonnull UUID playerUniqueId, @Nonnull PlayerSettings playerSettings) {
		return new PlayerEventPacket(PlayerEventType.PROXY_LOGIN_SUCCESS, buffer -> buffer.writeUUID(playerUniqueId).writeObject((SerializableObject) playerSettings));
	}

	public static PlayerEventPacket forProxyServerConnectRequest(@Nonnull UUID playerUniqueId, @Nonnull UUID targetUniqueId) {
		return new PlayerEventPacket(PlayerEventType.PROXY_SERVER_CONNECT_REQUEST, buffer -> buffer.writeUUID(playerUniqueId).writeUUID(targetUniqueId));
	}
	...
	public static PlayerEventPacket forProxyServerSwitch(@Nonnull UUID playerUniqueId, @Nonnull UUID fromUniqueId, @Nonnull UUID toUniqueId) {
		return new PlayerEventPacket(PlayerEventType.PROXY_SERVER_SWITCH, buffer -> buffer.writeUUID(playerUniqueId).writeUUID(fromUniqueId).writeUUID(toUniqueId));
	}

	public static PlayerEventPacket forProxyDisconnect(@Nonnull UUID playerUniqueId) {
		return new PlayerEventPacket(PlayerEventType.PROXY_DISCONNECT, buffer -> buffer.writeUUID(playerUniqueId));
	}

	public static PlayerEventPacket forServerLoginRequest(@Nonnull UUID playerUniqueId) {
		return new PlayerEventPacket(PlayerEventType.SERVER_LOGIN_REQUEST, buffer -> buffer.writeUUID(playerUniqueId));
	}

	public static PlayerEventPacket forServerLoginRequest(@Nonnull UUID playerUniqueId, @Nonnull UUID serviceUniqueId) {
		return new PlayerEventPacket(PlayerEventType.SERVER_LOGIN_REQUEST, buffer -> buffer.writeUUID(playerUniqueId).writeUUID(serviceUniqueId));
	}

	public static PlayerEventPacket forServerLoginSuccess(@Nonnull UUID playerUniqueId) {
		return new PlayerEventPacket(PlayerEventType.SERVER_LOGIN_SUCCESS, buffer -> buffer.writeUUID(playerUniqueId));
	}

	public static PlayerEventPacket forServerLoginSuccess(@Nonnull UUID playerUniqueId, @Nonnull UUID serviceUniqueId) {
		return new PlayerEventPacket(PlayerEventType.SERVER_LOGIN_SUCCESS, buffer -> buffer.writeUUID(playerUniqueId).writeUUID(serviceUniqueId));
	}

	public static PlayerEventPacket forServerDisconnect(@Nonnull UUID playerUniqueId) {
		return new PlayerEventPacket(PlayerEventType.SERVER_DISCONNECT, buffer -> buffer.writeUUID(playerUniqueId));
	}

	public static PlayerEventPacket forServerDisconnect(@Nonnull UUID playerUniqueId, @Nonnull UUID serviceUniqueId) {
		return new PlayerEventPacket(PlayerEventType.SERVER_DISCONNECT, buffer -> buffer.writeUUID(playerUniqueId).writeUUID(serviceUniqueId));
	}

	public static PlayerEventPacket forPlayerSettingsChange(@Nonnull UUID playerUniqueId, @Nonnull PlayerSettings settings) {
		return new PlayerEventPacket(PlayerEventType.PLAYER_SETTINGS_CHANGE, buffer -> buffer.writeUUID(playerUniqueId).writeObject((SerializableObject) settings));
	}

	protected PlayerEventPacket(@Nonnull PlayerEventType type, @Nonnull Consumer<? super Buffer> modifier) {
