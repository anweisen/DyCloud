...
public class RemoteTemplateStorage implements TemplateStorage, NetworkingApiUser {
	...
	public Task<Collection<ServiceTemplate>> getTemplatesAsync() {
		return sendPacketQueryAsync(new TemplateStoragePacket(TemplateStoragePayload.GET_TEMPLATES, buffer -> buffer.writeString(name)))
	...
	public Task<InputStream> zipTemplateAsync(@Nonnull ServiceTemplate template) {
		...
		return sendChunkedPacketQuery(
			new TemplateStoragePacket(TemplateStoragePayload.LOAD_TEMPLATE_STREAM, buffer -> buffer.writeObject(template))
		).map(response -> {
	...
	public Task<Boolean> hasTemplateAsync(@Nonnull ServiceTemplate template) {
		return sendPacketQueryAsync(new TemplateStoragePacket(TemplateStoragePayload.HAS_TEMPLATE, buffer -> buffer.writeObject(template)))
