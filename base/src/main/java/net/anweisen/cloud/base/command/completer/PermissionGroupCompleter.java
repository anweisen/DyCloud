 ...
 * @since 1.0
 */
public class PermissionGroupCompleter implements CommandCompleter {

	@Nonnull
	@Override
	public Collection<String> complete(@Nonnull CommandSender sender, @Nonnull String message, @Nonnull String argument) {
		return CloudDriver.getInstance().getPermissionManager().getGroups().stream().map(PermissionGroup::getName).collect(Collectors.toList());
	}
}
